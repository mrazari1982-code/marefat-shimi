-- Atomic expiry finalization. No historical expired attempts are converted.
create schema if not exists v5_private;
revoke all on schema v5_private from public, anon, authenticated;
...
as $$ select least(
 ...
 e.end_at)
 from public.v5_attempts a join public.v5_exams e on e.id=a.exam_id where a.id=p_attempt_id $$;
revoke all on function v5_private.attempt_deadline(uuid) from public,anon,authenticated;

create or replace function v5_private.finalize_attempt(p_attempt_id uuid)
returns public.v5_attempts language plpgsql security invoker set search_path = ''
as $$
...
revoke all on function v5_private.finalize_attempt(uuid) from public,anon,authenticated;
...
revoke all on function v5_private.finalize_due_attempts() from public,anon,authenticated;
...
revoke all on function public.v5_submit_attempt(uuid,text) from public;
grant execute on function public.v5_submit_attempt(uuid,text) to anon,authenticated,service_role;
...
revoke all on function public.v5_save_answer(uuid,bigint,bigint,text) from public;
revoke all on function public.v5_save_answers(uuid,text,jsonb) from public;
grant execute on function public.v5_save_answer(uuid,bigint,bigint,text) to anon,authenticated,service_role;
grant execute on function public.v5_save_answers(uuid,text,jsonb) to anon,authenticated,service_role;
...
select cron.schedule('v5-finalize-due-attempts','30 seconds','select v5_private.finalize_due_attempts();');
